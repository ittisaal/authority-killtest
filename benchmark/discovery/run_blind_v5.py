...
def summarize(rows):
    ...
    for row in rows:
        ...
        for candidate in row.get("candidates", []):
            ...
            raw_v4_labels[candidate.get("raw_v4_label", candidate["label"])] += 1
    return {
        "cases": len(rows),
        ...
        "raw_v4_candidate_labels": {k: raw_v4_labels.get(k, 0) for k in ("U", "S", "N")},
        "note": "Raw blind-v5 target-blind outputs. Candidate counts are not prevalence and are not scored against withheld targets here.",
    }
...
def main():
    ...
    for case in manifest["cases"]:
        ...
        if overlap:
            raise SystemExit(f"blind input leaks scoring fields for {case.get('id')}: {sorted(overlap)}")
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as pool:
        ...
        for future in as_completed(futures):
            row = future.result()
            rows.append(row)
            print(row["id"], row["repository"], "status=", row["status"], "candidates=", row.get("candidate_count", 0), flush=True)
    rows.sort(key=lambda row: row["id"])

    result = {
        "experiment": manifest.get("experiment", "blind-v5"),
