...
HERE = Path(__file__).with_name("discover_repository.py")
spec = importlib.util.spec_from_file_location("discover_repository", HERE)
...
def evaluate_case(case):
    ...
    target_key = discovery.canonical(case["target_property"])
...
def summarize(rows):
    ...
    return {
        ...
        "note": (
            ...
            "afterward for evaluation and are not passed into discovery. This is still a bounded snapshot test, "
            "not ecosystem prevalence or private-organization discovery."
        ),
    }
